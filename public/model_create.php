...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['name']) || empty($data['manufacturer_id']) || empty($data['category_id'])) {
        ...
        if ($masterData->addAssetModel($data)) {
            header('Location: settings.php?success=' . urlencode('Asset-Modell erfolgreich angelegt.'));
            exit;
        } else {
            $error = "Fehler beim Anlegen des Modells.";
        }
    }
}
?>
...
<head>
    ...
    <title>Asset-Modell anlegen - Mini-Snipe</title>
...
<body class="<?php echo ($_COOKIE['theme'] ?? 'dark') === 'light' ? 'light-mode' : ''; ?>">
    ...
    <main class="main-content">
        <header class="header">
            <h1>Neues Asset-Modell anlegen</h1>
                    ...
                    <div class="form-group">
                        ...
                        <select name="manufacturer_id" class="form-control" required>
                            ...
                            <?php foreach ($manufacturers as $man): ?>
                                <option value="<?php echo $man['id']; ?>" <?php echo ((isset($_POST['manufacturer_id']) && $_POST['manufacturer_id'] == $man['id']) ? 'selected' : ''); ?>>
